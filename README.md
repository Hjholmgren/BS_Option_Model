...
model. Built with Streamlit. Prices calls and puts, shows the full Greek
suite, solves for implied volatility, and visualises how the option behaves
as the inputs move.
...
1. Push this repo to GitHub (must include `dashboard.py`, `bs_functions.py`, and `requirements.txt`).
...
```
bs_functions.py   # the maths: prices, Greeks, implied vol (no UI code)
